...
begin
  ...
  v_OldTotalFee := 0;
  v_Stage       := '取出流程信息';
  select f.fillusrgid, f.filldeptgid, nvl(f.BRCBudget, 0), f.OldFlowGid
    into v_UsrGid, v_DeptGid, v_TotalFee, v_OldFlowGid
    from wf_PrlGG_ISF f
  ...
  select count(*)
    into v_Count
    from wf_PrlGG_ISF_Dtl t
   where t.entgid = p_EntGid
     and t.flowgid = p_FlowGid
     and t.TbID = 'tb_Dtl30';
  if v_Count > 0 then
    select v_TotalFee - nvl(t.TBRD, 0)
      into v_TotalFee
      from wf_PrlGG_ISF_Dtl t
     where t.entgid = p_EntGid
       and t.flowgid = p_FlowGid
  ...
  select round(v_TotalFee * 365 / 12, 2) into v_TotalFee from dual;
    ...
    insert into wf_PrlGG_ISF_App
      ...
      select p_EntGid,
       ...
       where v.EntGid = p_EntGid
         ...
         and v.atype = 71
         and rownum = 1
         and v_TotalFee - v_OldTotalFee > 0
      union
      select p_EntGid,
             p_FlowGid,
             sys_guid(),
