create or replace procedure HDNET_Prl_doOISF(p_EntGid  varchar2, --企业Gid
  ...
  v_YearFee number(20, 4); --第一年首年不含免租期的总租金单价
  v_FreeFee number(20, 4); --第一年首年含免租期的总租金单价
  v_FreeDay number(20, 4); --免租期天数

  v_Old_YearFee number(20, 4); --第一年首年不含免租期的总租金单价
  v_Old_FreeFee number(20, 4); --第一年首年含免租期的总租金单价
...
begin
  ...
  select nvl(f.sPARAM1, 0),
         ...
         Category,
         nvl(LeaseTermM, 0),
         nvl(f.FreeRentY, 0) * 365 + nvl(f.FreeRentM, 0) * 365 / 12 +
         nvl(f.FreeRentD, 0)
    into v_P1,
         v_P2,
         v_P3,
  ...
  select count(*)
    into v_Count
    from wf_PRL_OISF_Dtl fd
   where fd.EntGid = p_EntGid
     and fd.FlowGid = p_FlowGid
     and lower(fd.TbID) = 'tb_dtl30';
  if v_Count != 0 then
    select nvl(TBRD, 0), nvl(FTBRD, 0)
      into v_YearFee, v_FreeFee
      from wf_PRL_OISF_Dtl fd
     where fd.EntGid = p_EntGid
       and fd.FlowGid = p_FlowGid
       and lower(fd.TbID) = 'tb_dtl30'
  ...
  if v_OldFlowGid is not null then
    ...
    select nvl(f.sPARAM3, 0),
           nvl(f.TotalFee + f.OtherTotalFee, 0),
           nvl(f.Area, 0),
           f.ContractDate1,
           nvl(f.FreeRentY, 0) * 365 + nvl(f.FreeRentM, 0) * 365 / 12 +
           nvl(f.FreeRentD, 0)
      into v_Old_P3, v_Old_P4, v_Old_P6, v_Old_CBDate, v_Old_FreeDay
      from wf_PRL_OISF f
     where f.EntGid = p_EntGid
       and f.FlowGid = v_OldFlowGid;
  
    v_Stage       := '取出Old ISF' || v_OldFlowGid || '取出第一年租金';
    v_Old_FreeDay := 0;
    select count(*)
      into v_Count
      from wf_PRL_OISF_Dtl fd
    ...
    if v_Count != 0 then
      select nvl(TBRD, 0), nvl(FTBRD, 0)
        into v_Old_YearFee, v_Old_FreeFee
        from wf_PRL_OISF_Dtl fd
       where fd.EntGid = p_EntGid
         and fd.FlowGid = v_OldFlowGid
         and lower(fd.TbID) = 'tb_dtl30'
  ...
  if v_D1 = '固定' then
    ...
    if (v_P1 > 10 or v_P2 > 50000) or v_P4 > 80000 or (v_P6 > 500 and v_P3 > v_T3) then
      v_Result := 90;
    end if;
