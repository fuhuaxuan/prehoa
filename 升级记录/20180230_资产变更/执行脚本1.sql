...
--4、定义工作流程名称 ，步骤等
  ...
  select v_EntGid, v_ModelGid, v_ModelCode, '固定资产变更申请流程', 3, '1.审批流程', '1.0', 0, 0 from dual;
...
drop table WF_PrlDZ_Change;			
create table WF_PrlDZ_Change (			
	...
	ChgType	varchar2(16)	null,
	ChgQty	int	null,
	AssetNum	varchar2(32)	null,
	AssetName	varchar2(128)	null,
	AssetPlace	varchar2(256)	null,
	OpResult	varchar2(2000)	null,
	ReceiveGid	varchar2(32)	null,
	ReceiveCode	varchar2(64)	null,
	ReceiveName	varchar2(64)	null,
	Memo	varchar2(2000)	null,
	constraint PK_WF_PrlDZ_Change primary key(EntGid, FlowGid),		
	CONSTRAINT UNQ_PrlDZ_Change UNIQUE(Num)		
	);		
create index idx_WF_PrlDZ_Change_1 on WF_PrlDZ_Change(FillUsrGid);			
			
drop table WF_PrlDZ_Change_App;			
create table WF_PrlDZ_Change_App (			
	EntGid 	varchar2(32)	not null,
	ModelGid	varchar2(32)	not null,
	FlowGid	varchar2(32)	not null,
	...
	AppDate	date	null,
	constraint PK_WF_PrlDZ_Change_App primary key(EntGid, FlowGid, Gid)		
	);		
			
			
drop table WF_PrlDZ_Change_Attach;			
create table WF_PrlDZ_Change_Attach (			
	EntGid 	varchar2(32)	not null,
	ModelGid	varchar2(32)	not null,
	FlowGid	varchar2(32)	not null,
	Gid	varchar2(32)	not null,
	...
	AttachSize	int	null,
	constraint PK_WF_PrlDZ_Change_Attach primary key(EntGid, FlowGid, Gid)		
	);		
			
			
			
create or replace procedure P_PrlDZ_Change_doApp(p_EntGid    varchar2, --企业Gid
...
begin
  ...
  select f.FillUsrGid,
         ...
         f.Comgid
    into v_UsrGid, v_DeptGid, v_PreDeptCode, v_ComGid
    from wf_PrlDZ_Change f
  ...
  if p_AppAssign <> '终止' then
    --插入审批人
    insert into wf_PrlDZ_Change_App
      (EntGid,
       ModelGid,
       FlowGid,
       Gid,
        ...
        from (select v.PostGid  AppGid,
              ...
              select v.PostGid  AppGid,
               ...
               where v.EntGid = p_EntGid
                 ...
                 and rownum = 1
              union
              select v.ReceiveGid  AppGid,
                     v.ReceiveCode AppCode,
                     v.ReceiveName AppName,
                     35            AppOrder,
                     35            AppType
                from wf_PrlDZ_Change v
               where v.EntGid = p_EntGid
                 and v.FlowGid = p_FlowGid
                 and v.ChgType = '转移'
                 and v.ReceiveGid is not null
                 and rownum = 1
              union
              select v.PostGid  AppGid,
                     v.PostCode AppCode,
    ...
    --取出审批人中重复的审批人
    delete from wf_PrlDZ_Change_App f
     where f.EntGid = p_EntGid
       and f.FlowGid = p_FlowGid
       and f.Apporder > 0
...
/
create or replace procedure P_PrlDZ_Change_doMail(p_FlowGid varchar --流程Gid
                                                  ) as
  v_Stage   varchar2(1024); -- 过程场景
  v_ErrText varchar2(1024); -- Oracle错误信息
...
begin
  ...
  for R in (select f.*, wm.name ModelName
    ...
    v_Content := v_Content || v_TStart || v_Body || v_TEnd;
    v_Body    := '[变更类型] : ' || R.ChgType;
    v_Content := v_Content || v_TStart || v_Body || v_TEnd;
    v_Body    := '[资产编号] : ' || R.AssetNum;
    v_Content := v_Content || v_TStart || v_Body || v_TEnd;
    v_Body    := '[资产名称] : ' || R.AssetName;
    v_Content := v_Content || v_TStart || v_Body || v_TEnd;
    v_Body    := '[数量] : ' || R.ChgQty;
    v_Content := v_Content || v_TStart || v_Body || v_TEnd;
    v_Body    := '[存放地点] : ' || R.AssetPlace;
    v_Content := v_Content || v_TStart || v_Body || v_TEnd;
    v_Body    := '[详细原因说明] : ' || R.Memo;
    v_Content := v_Content || v_TStart || v_Body || v_TEnd;
    v_Body    := '[处理结果] : ' || R.OpResult;
    v_Content := v_Content || v_TStart || v_Body || v_TEnd;
