...
--4、定义工作流程名称 ，步骤等
  ...
  select v_EntGid, v_ModelGid, v_ModelCode, '广告位SOS审批流程', 3, '项目审批流程-租赁相关', '1.0', 0, 0 from dual;
...
drop table WF_PrlGG_Sos;			
create table WF_PrlGG_Sos (			
	...
	fmrMSum1	NUMBER(24,2)	null,
	fAreaSum2	NUMBER(24,2)	null,
	jAreaSum2	NUMBER(24,2)	null,
	fmrSum2	NUMBER(24,2)	null,
	fmrMSum2	NUMBER(24,2)	null,
	Memo	varchar2(2000)	null,
	constraint PK_WF_PrlGG_Sos primary key(EntGid, FlowGid),		
	CONSTRAINT UNQ_PrlGG_Sos UNIQUE(Num)		
	);		
create index idx_WF_PrlGG_Sos_1 on WF_PrlGG_Sos(FillUsrGid);			
			
drop table WF_PrlGG_Sos_Dtl;			
create table WF_PrlGG_Sos_Dtl (			
	EntGid 	varchar2(32)	not null,
	ModelGid	varchar2(32)	not null,
	FlowGid	varchar2(32)	not null,
	...
	fmrY	NUMBER(24,2)	null,
	constraint PK_WF_PrlGG_Sos_Dtl primary key(EntGid, FlowGid, Gid)		
	);		
			
drop table WF_PrlGG_Sos_App;			
create table WF_PrlGG_Sos_App (			
	EntGid 	varchar2(32)	not null,
	ModelGid	varchar2(32)	not null,
	FlowGid	varchar2(32)	not null,
	Gid	varchar2(32)	not null,
	...
	AppDate	date	null,
	constraint PK_WF_PrlGG_Sos_App primary key(EntGid, FlowGid, Gid)		
	);		
			
			
drop table WF_PrlGG_Sos_Attach;			
create table WF_PrlGG_Sos_Attach (			
	EntGid 	varchar2(32)	not null,
	ModelGid	varchar2(32)	not null,
	FlowGid	varchar2(32)	not null,
	Gid	varchar2(32)	not null,
	...
	AttachSize	int	null,
	constraint PK_WF_PrlGG_Sos_Attach primary key(EntGid, FlowGid, Gid)		
	);		

create or replace procedure P_PrlGG_Sos_doApp(p_EntGid    varchar2, --企业Gid
...
begin
  ...
  if p_AppAssign <> '终止' then
    ...
    --取出审批人中重复的审批人
    delete from wf_PrlGG_Sos_App f
     where f.EntGid = p_EntGid
       and f.FlowGid = p_FlowGid
       and f.Apporder > 0
...
create or replace procedure P_PrlGG_SOS_doMail(p_FlowGid varchar --流程Gid
                                               ) as
  v_Stage   varchar2(1024); -- 过程场景
  v_ErrText varchar2(1024); -- Oracle错误信息
...
begin
  ...
  for R in (select f.*, wm.name ModelName
              from wf_PrlGG_Sos f, wf_model wm
    ...
    if R.ApplyType = '变更' then
      ...
      v_Body    := v_Body || '    <td>最小面积</td>';
      v_Body    := v_Body || '    <td>最大面积</td>';
      v_Body    := v_Body || '    <td>预算单价</td>';
      v_Body    := v_Body || '  </tr>';
      v_Body    := v_Body || '  </thead>';
      v_Body    := v_Body || '  <tbody>';
      v_Content := v_Content || v_TStart || v_Body || v_TEnd;
...
alter table PrlGG_GROUNDH add OPFlowGid varchar2(32);
alter table PrlGG_GROUNDH add OPNum varchar2(32);