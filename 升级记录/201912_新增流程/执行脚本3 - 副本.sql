...
alter table wf_PrlDZ_Area_Dtl add AddMemo  varchar2(2000);
alter table wf_PrlDZ_Area_Dtl add ChgMemo  varchar2(2000);
...
begin
  ...
  for R in (select f.*, wm.name ModelName
    ...
    v_Content := v_Content || v_TStart || v_Body || v_TEnd;
    v_Body    := '[申报日期] : ' || to_char(R.AddDate, 'YYYY.MM.DD');
    v_Content := v_Content || v_TStart || v_Body || v_TEnd;
    v_Body    := '地上信息 : ';
    v_Content := v_Content || v_TStart || v_Body || v_TEnd;
...
begin
  ...
  for R in (select f.*, wm.name ModelName
    ...
    v_Body    := '[申报日期] : ' || to_char(R.AddDate, 'YYYY.MM.DD');
    v_Content := v_Content || v_TStart || v_Body || v_TEnd;
    v_Body    := '[变更日期] : ' || to_char(R.ChgDate, 'YYYY.MM.DD');
    v_Content := v_Content || v_TStart || v_Body || v_TEnd;
    v_Body    := '地上信息 : ';
    v_Content := v_Content || v_TStart || v_Body || v_TEnd;
    v_Body    := '<table cellpadding="0" cellspacing="1" class="ListBar" width="100%" style="background-color: #d9dbdf;">';
    ...
    v_Body    := v_Body || '<td>项目</td>';
    v_Body    := v_Body || '<td>申报面积</td>';
    v_Body    := v_Body || '<td>变更面积</td>';
    v_Body    := v_Body || '<td>差异</td>';
    v_Body    := v_Body || '<td>申报备注</td>';
    v_Body    := v_Body || '<td>变更备注</td>';
    v_Body    := v_Body || '</tr>';
    v_Content := v_Content || v_TStart || v_Body;
    ...
    v_Body    := '[申报备注] : ' || R.AddMemo;
    v_Content := v_Content || v_TStart || v_Body || v_TEnd;
    v_Body    := '[变更备注] : ' || R.ChgMemo;
    v_Content := v_Content || v_TStart || v_Body || v_TEnd;
