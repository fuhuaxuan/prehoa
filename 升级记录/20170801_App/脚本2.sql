...
create or replace procedure P_Prl_OISCF_doMail(p_FlowGid varchar --流程Gid
                                                ) as
begin
  P_Prl_OISF_doMail(p_FlowGid);
end;
/
create or replace procedure P_Prl_OISTF_doMail(p_FlowGid varchar --流程Gid
                                               ) as
  v_Stage   varchar2(1024); -- 过程场景
  v_ErrText varchar2(1024); -- Oracle错误信息
...
begin
  ...
  for R in (select f.*,
                   ...
                   t.fitoutdate2,
                   wm.name ModelName
              from wf_Prl_OISTF f, wf_Prl_OISF t, wf_model wm
             where f.EntGid = v_EntGid
               ...
               and f.modelgid = wm.modelgid) loop
    v_Stage   := 'FlowGid：' || R.Flowgid || '；模型：' || R.ModelName;
    v_Title   := '写字楼ISTF待审提醒:' || R.Filldeptname;
    v_Content := v_Content || v_Head;
