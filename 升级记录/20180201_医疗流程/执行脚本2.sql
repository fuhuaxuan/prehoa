create or replace procedure P_PrlYY_Baoxiao_doApp(p_EntGid    varchar2, --企业Gid
...
begin
  ...
  if p_AppAssign <> '终止' then
    --插入审批人
    insert into wf_PrlYY_Baoxiao_App
      (EntGid,
       ModelGid,
       FlowGid,
       gid,
    ...
    commit;
    --取出审批人中重复的审批人
    delete from wf_PrlYY_Baoxiao_App f
     where f.EntGid = p_EntGid
       and f.FlowGid = p_FlowGid
       and f.Apporder > 0
...
create or replace procedure P_PrlYY_Fee_doApp(p_EntGid    varchar2, --企业Gid
  ...
  if p_AppAssign <> '终止' then
    ...
    insert into wf_PrlYY_Fee_App
      ...
      select p_EntGid,
       ...
       where t.EntGid = p_EntGid
         and t.ComGid = v_ComGid
         and lower(t.modelcode) = 'PrlYY_fee'
         and t.AppGid is not null
         and exists (select 1
                from PrlYY_Acg_Post p
               where p.entgid = t.entgid
    ...
    commit;
    --取出审批人中重复的审批人
    delete from wf_PrlYY_Fee_App f
     where f.EntGid = p_EntGid
       and f.FlowGid = p_FlowGid
       and f.Apporder > 0
       and f.Appdate is null
...
/
create or replace procedure P_PrlYY_Pay_doApp(p_EntGid    varchar2, --企业Gid
                                              p_ModelGid  varchar, --模型Gid
                                              p_FlowGid   varchar, --流程Gid
                                              p_AppAssign varchar2 --意见
                                              ) as
  ...
  if p_AppAssign <> '终止' then
    ...
    insert into wf_PrlYY_Pay_App
      ...
      select p_EntGid,
       ...
       where t.EntGid = p_EntGid
         and t.ComGid = v_ComGid
         and lower(t.modelcode) = 'PrlYY_pay'
         and t.AppGid is not null
         and exists (select 1
                from PrlYY_Acg_Post p
               where p.entgid = t.entgid
    ...
    commit;
    --取出审批人中重复的审批人
    delete from wf_PrlYY_Pay_App f
     where f.EntGid = p_EntGid
       and f.FlowGid = p_FlowGid
       and f.Apporder > 0
       and f.Appdate is null
...
/
create or replace procedure P_PrlYY_Stamp_doApp(p_EntGid    varchar2, --企业Gid
                                              p_ModelGid  varchar, --模型Gid
                                              p_FlowGid   varchar, --流程Gid
                                              p_AppAssign varchar2 --意见
                                              ) as
...
begin
  ...
  if p_AppAssign <> '终止' then
    --插入审批人
    insert into wf_PrlYY_Stamp_App
      (EntGid,
       ModelGid,
       FlowGid,
       Gid,
    ...
    --取出审批人中重复的审批人
    delete from wf_PrlYY_Stamp_App f
     where f.EntGid = p_EntGid
       and f.FlowGid = p_FlowGid
       and f.Apporder > 0
       and f.Appdate is null
