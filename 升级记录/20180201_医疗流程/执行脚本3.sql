create or replace procedure P_PrlYY_BaoXiao_doMail(p_FlowGid varchar --流程Gid
                                                   ) as
...
begin
  ...
  for R in (select f.*, wm.name ModelName
              from Wf_PrlYY_Baoxiao f, wf_model wm
...
/
create or replace procedure P_PrlYY_Fee_doMail(p_FlowGid varchar --流程Gid
                                               ) as
  v_Stage   varchar2(1024); -- 过程场景
  v_ErrText varchar2(1024); -- Oracle错误信息
...
begin
  ...
  --for 循环 取出未领取的快递
  for R in (select f.*, wm.name ModelName
              from wf_PrlYY_Fee f, wf_model wm
             where f.EntGid = v_EntGid
               and f.entgid = wm.entgid
               and f.FlowGid = p_FlowGid
               and f.modelgid = wm.modelgid) loop
...
/
create or replace procedure P_PrlYY_Pay_doMail(p_FlowGid varchar --流程Gid
                                               ) as
  v_Stage   varchar2(1024); -- 过程场景
  v_ErrText varchar2(1024); -- Oracle错误信息
...
begin
  ...
  --for 循环 取出未领取的快递
  for R in (select f.*, wm.name ModelName
              from wf_PrlYY_Pay f, wf_model wm
             where f.EntGid = v_EntGid
               and f.entgid = wm.entgid
               and f.FlowGid = p_FlowGid
               and f.modelgid = wm.modelgid) loop
...
/
create or replace procedure P_PrlYY_Stamp_doMail(p_FlowGid varchar --流程Gid
                                               ) as
  v_Stage   varchar2(1024); -- 过程场景
  v_ErrText varchar2(1024); -- Oracle错误信息
...
begin
  ...
  --for 循环 取出未领取的快递
  for R in (select f.*, wm.name ModelName
              from wf_PrlYY_Stamp f, wf_model wm
             where f.EntGid = v_EntGid
               and f.entgid = wm.entgid
               and f.FlowGid = p_FlowGid
               and f.modelgid = wm.modelgid) loop
