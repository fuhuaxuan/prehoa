...
select f.entgid, f.flowgid, f.applyfee,f.payman,f.paybank,f.payaccount
  from v_prl_flow f
union
select f.entgid, f.flowgid, f.applyfee,f.payman,f.paybank,f.payaccount
  from v_prlzb_flow f;

create or replace procedure P_PrlDZ_Baoxiao_doPay(p_EntGid    varchar2, --企业Gid
...
begin
  if lower(p_ModelCode) = lower('PrlDZ_Baoxiao') and p_FlowGid is not null then
    v_Stage := '取出用户信息';
    ...
    select TaskDefGid
     ...
     where EntGid = p_EntGid
       ...
       and lower(Code) = lower('PrlDZ_Baoxiao_Tcc');
    ...
    select TaskDefGid
     ...
     where EntGid = p_EntGid
       and ModelGid = p_ModelGid
       and lower(Code) = lower('PrlDZ_Baoxiao_t4')
       and IsMcf = 0;
